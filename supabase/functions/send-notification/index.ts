...
// Email uses the primary credential first, then optionally switches to a backup provider.
async function sendEmail(recipient: string, body: string) {
  const send = async (apiKey: string | undefined, from: string | undefined) => {
  ...
  let provider = "resend";
  if (!response.ok && Deno.env.get("RESEND_BACKUP_API_KEY")) {
    ({ response, result } = await send(
      Deno.env.get("RESEND_BACKUP_API_KEY"),
      Deno.env.get("RESEND_BACKUP_FROM_EMAIL") ??
        Deno.env.get("RESEND_FROM_EMAIL"),
    ));
    provider = "resend-backup";
  }
  if (!response.ok)
    throw new Error(result.message ?? "Email provider rejected the request");
  return { provider, providerMessageId: result.id as string };
}
...
async function sendPush(recipient: string, body: string) {
  ...
  const response = await fetch(
    `https://fcm.googleapis.com/v1/projects/${projectId}/messages:send`,
    {
      ...
      body: JSON.stringify({
        message: {
          token: recipient,
          notification: { title: "Notification service test", body },
        },
      }),
    },
  );
  ...
  if (!response.ok)
    throw new Error(
      result.error?.message ?? "Push provider rejected the request",
    );
...
Deno.serve(async (req) => {
  ...
  if (!user)
    return reply({ error: "Sign in is required to send notifications." }, 401);
  const {
    channel,
    recipient,
    body,
  ...
  if (
    !["email", "sms", "push"].includes(channel) ||
    !recipient ||
    !body ||
    !idempotencyKey
  )
    return reply({ error: "Invalid notification payload." }, 400);
  // The service client bypasses RLS only inside this trusted server-side function.
  ...
  const { data: duplicate } = await db
    ...
    .maybeSingle();
  if (duplicate) return reply({ notification: duplicate, duplicate: true });
  const priority = trafficClass === "bulk" ? "bulk" : "transactional";
  ...
  if (insertError) {
    const { data: existing } = await db
      ...
      .maybeSingle();
    return existing
      ? reply({ notification: existing, duplicate: true })
      : reply({ error: insertError.message }, 500);
  }
  // Persist each attempt, wait with exponential backoff, and stop after five failures.
  for (let attempt = 1; attempt <= 5; attempt++) {
    ...
    try {
      ...
      const { data } = await db
        ...
        .select()
        .single();
      return reply({ notification: data, attempts: attempt });
    } catch (error) {
      const message =
